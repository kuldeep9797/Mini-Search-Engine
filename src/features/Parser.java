...
public class Parser 
{
	...
	private static Set<String> stopWords = new HashSet<String>(Arrays.asList("a,the,and,are,an,as,at,be,by,for,from,has,he,is,its,of,on,it,that,was,to,were,will,with".split(",")));
	...
	public static void generateFilesList() 
	{
		try 
		{
			File directory = new File(Resources.txtDirectoryPath);
	...
	public static void saveDoc(Document doc) 
	{
		try 
		{
			BufferedWriter html = new BufferedWriter(
					new FileWriter(Resources.htmlDirectoryPath + doc.title().replace('/', '-') + ".html"));
			html.write(doc.toString());
			html.close();

			BufferedWriter txt = new BufferedWriter(
					new FileWriter(Resources.txtDirectoryPath + doc.title().replace('/', '-') + ".txt"));
		...
		{
			System.out.println(
					"Exception in saving file: " + Resources.txtDirectoryPath + doc.title().replace('/', '-') + ".txt");
		}
	}
}
